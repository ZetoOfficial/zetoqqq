...
| `wedding.zetoqqq.ru` | docker, GHCR | `127.0.0.1:8080`, `:8081`, `:8082` |
| `tax-residency-calculator.zetoqqq.ru` | docker, GHCR | `127.0.0.1:8091` |
| `code.zetoqqq.ru` | rustpad | `127.0.0.1:3030` |

Отсюда следствие, которое стоит держать в голове: **ошибка в nginx-конфиге
кладёт не только этот сайт, а все четыре.** Поэтому любая правка — только
через `nginx -t` до `systemctl reload nginx` (см. раздел 5).
...
и что `/` отдаёт настоящий HTML — а не пустой 200, ровно так выглядела заглушка,
...
**`nginx -t` перед reload — не формальность.** Конфиг общий на все четыре сайта
